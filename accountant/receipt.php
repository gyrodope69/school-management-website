<?php 
include '../config.php';
$fees = $conn->query("SELECT ef.*,s.name as sname,s.student_id,c.standard as `class` FROM student_ef_list ef inner join students s on s.student_id = ef.student_id inner join classes c on c.class_id = ef.course_id  where ef.id = {$_GET['ef_id']}");
//$fees = $conn->query("SELECT ef.*,s.name as sname,s.student_id FROM student_ef_list ef inner join students s on s.student_id = ef.student_id where ef.id = {$_GET['ef_id']}") or die('query failed.');
